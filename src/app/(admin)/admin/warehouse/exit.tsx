...
export function AdminWarehouseExit({ products, stock, onSync }: { products: Product[]; stock: WholesalerStockItem[], onSync: (scannedItems: Record<string, number>) => void }) {
  ...
  const handleConfirmScan = () => {
    ...
    toast({ title: "Επιτυχής Καταχώρηση Εξόδου!", description: `Προστέθηκε: ${product.name}` });

    const element = document.getElementById(`exit-product-${product.id}`);
      ...
      <Card onClick={() => setIsScanning(true)} className="cursor-pointer bg-card overflow-hidden group">
        <div className="relative h-36">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-4">
            ...
            <h3 className="text-white font-bold text-xl drop-shadow-md">Σκανάρισμα για Έξοδο</h3>
            <p className="text-white/80 text-sm drop-shadow-sm">Σαρώστε προϊόντα για να τα αφαιρέσετε από το απόθεμα</p>
          </div>
        </div>
      </Card>
        ...
        <DialogContent>
            ...
            <>
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Το προϊόν βρέθηκε. Πατήστε "Επιβεβαίωση" για να αυξήσετε την καταμέτρηση εξόδου.
                    </DialogDescription>
                </DialogHeader>
