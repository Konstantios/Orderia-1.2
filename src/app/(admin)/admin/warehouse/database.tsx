...
import { Plus, Camera, ScanLine, X, Save, Package } from 'lucide-react';
...
export function AdminWarehouseDatabase({ products, wholesaler }: { products: Product[], wholesaler?: Wholesaler }) {
  ...
  const addProduct = async (newProduct: Product) => {
    ...
    try {
      const colRef = collection(firestore, 'wholesalers', wholesaler.id, 'products');
      const { id, ...data } = newProduct; // Don't save the temporary ID
      await addDoc(colRef, {
        ...data,
        ...
        isScanBaseHelper: true // Mark as added via scan base for possible filtering
      });
    } catch (error) {
      console.error("Error adding product to official database:", error);
    }
  };
          ...
          {filteredProducts.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {filteredProducts.map(product => (
                <Card key={product.id} className="bg-card/50 cursor-pointer hover:bg-card/70 transition-colors" onClick={() => setSelectedProduct(product)}>
                  <div className="p-3 flex items-center gap-4">
                    {product.imageUrl ? (
                        <Image src={product.imageUrl} alt={product.name} width={64} height={64} className="rounded-md object-cover flex-shrink-0" data-ai-hint={product.imageHint} />
                    ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground truncate">Κωδικός: {product.code}</p>
                    </div>
                  </div>
                </Card>
              ))}
            ...
            <div className="text-center py-8 text-muted-foreground">
              <p>Δεν βρέθηκαν προϊόντα στη βάση δεδομένων.</p>
            </div>
          )}
        </CardContent>
      </Card>
          ...
          {selectedProduct && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>Κωδικός: {selectedProduct.code}</DialogDescription>
              </DialogHeader>
