...
export function InventoryEntry({ products, customer, inventory, productConfigs, isLoading, onSync }: { products: Product[]; customer: any; inventory: CustomerInventoryItem[], productConfigs?: StoreProductConfiguration[], isLoading?: boolean, onSync: (scannedItems: Record<string, number>) => void }) {
      ...
      <div className="space-y-3">
        {inventoryData
          ...
          .map(({ product, currentStock, idealStock }) => {
            ...
            return (
            <Card key={product.id} id={`entry-product-${product.id}`} className={cn(
